...
const advanced_typeform_elastic_map = require('./advanced_typeform_elastic_map').advanced_typeform_elastic_map

exports.process_advanced_form = function(typeform) {
  const p = new Promise((res, rej) => {
    if (verifyCorrectForm(typeform, advanced_typeform_elastic_map)) {
      /*
        ...
        6. Save to DynamoDB
      */
      const QnT = extractQuestionsAndTags(advanced_typeform_elastic_map)
      // console.log(QnT)
      const grouped = groupWithTypeform(QnT, typeform)
      res(grouped)
    } else {
      rej(`The received Typeform ID#${typeform.form_response.form_id} did not match the template map with ID#${advanced_typeform_elastic_map.form_id}`)
    }
  })
  return p
}
