...
const basic_typeform_elastic_map = require('./basic_typeform_elastic_map').basic_typeform_elastic_map

exports.process_basic_form = function(typeform) {
  const p = new Promise((res, rej) => {
    if (verifyCorrectForm(typeform, basic_typeform_elastic_map)) {
      /*
        ...
        6. Save to DynamoDB
      */
      const QnT = extractQuestionsAndTags(basic_typeform_elastic_map)
      // console.log(QnT)
      const grouped = groupWithTypeform(QnT, typeform)
      res(grouped)
    } else {
      rej(`The received Typeform ID#${typeform.form_response.form_id} did not match the template map with ID#${basic_typeform_elastic_map.form_id}`)
    }
  })
  return p
}
